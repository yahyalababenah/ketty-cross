...
import { LANE, COLS, TILE, leftX, rightX, rowToZ, rand, pick, rnd } from './util.mjs';
...
function makeSignal(){
  ...
  const lamp = new THREE.Mesh(new THREE.SphereGeometry(0.08,10,10), new THREE.MeshStandardMaterial({color:0xaa0000, emissive:0x220000, emissiveIntensity:0.2})); lamp.position.set(0.18,1.05,0); g.add(lamp);
...
function laneMaterial(type, mats){
  return type===LANE.ROAD? mats.matRoad : type===LANE.RIVER? mats.matRiver : type===LANE.RAIL? mats.matRail : type===LANE.FINISH? mats.matFinish : mats.matGrass;
}
export function makeLaneMesh(type, assets){
  const {geos:{geoLane}, mats, textures:{BARK_TEX}} = assets;
  const lane = new THREE.Mesh(geoLane, laneMaterial(type, mats)); lane.receiveShadow=true;
  ...
  if(type===LANE.GRASS){
    ...
    for(let i=0;i<n;i++){
      ...
      const leafMat = new THREE.MeshStandardMaterial({color:leafCols[Math.floor(Math.random()*leafCols.length)], roughness:0.7, metalness:0.02});
...
export function spawnEntities(lane, assets, world){
  ...
  if(lane.type===LANE.RIVER){
    ...
    must.position.set(0, 0.14, 0); world.add(must); lane.ents.push(must);
  }
...
export function clearEntities(lanes, world){
  lanes.forEach(l=>{ if(l.ents){ l.ents.forEach(e=>world.remove(e)); l.ents.length=0; } if(l.signal){ world.remove(l.signal); l.signal=null; } });
}
export function placeCoins(lanes){
  const s=new Set(); for(let r=1;r<lanes.length-1;r++){ if(rnd()<0.22 && (lanes[r].type===LANE.GRASS||lanes[r].type===LANE.ROAD)){ s.add(`${r}:${Math.floor(rand(0,COLS))}`);} } return s;
}
