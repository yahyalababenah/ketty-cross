...
export function makeRobot(State){
  ...
  // أبعاد تقريبًا حقيقية (KettyBot)
  const DIM = { W:0.435, D:0.450, H:1.120 };
  const DIAG_SCREEN_M = 18.5 * 0.0254;
  const k = Math.sqrt(16*16 + 9*9);
  const SCREEN0_W = DIAG_SCREEN_M*(16/k);
  const SCREEN0_H = DIAG_SCREEN_M*(9/k);
  const SCREEN_SCALE = 1.25;
  const SCRW = SCREEN0_W * SCREEN_SCALE;
  const SCRH = SCREEN0_H * SCREEN_SCALE;
  ...
  const baseH = 0.13;
  const baseDia = Math.min(DIM.W, DIM.D) * 0.92;
  ...
  shell.position.set(0, baseH/2 + shellH/2, 0);
  ...
  const fasT = 0.055;
  const fasR = Math.min(SCRW, SCRH) * 0.12;
  ...
  bezel.position.copy(fascia.position).add(new THREE.Vector3(-0.02, 0, 0));
  bodyGroup.add(bezel);
