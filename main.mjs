...
function updateParticles(dt){
  for(const p of particles){
    p.t+=dt;
    p.m.position.x += p.vx*dt/1000;
    p.m.position.y += p.vy*dt/1000;
    p.m.position.z += p.vz*dt/1000;
    p.vy -= 3*dt/1000;
...
function buildScene(){
  ...
  for(let r=0;r<State.lanes.length;r++){
    ...
    if(data.type===LANE.RAIL && data.signal){
      data.signal.position.z = rowToZ(r);
    }
  }
  ...
  robot = makeRobot(State); // يبني الروبوت حسب الجلد والقياسات
  robot.position.set(colToX(State.player.col),0.1,rowToZ(State.player.row));
  robot.rotation.y = State.player.yaw; // يواجه +Z
  world.add(robot);
  ...
  for(let r=0;r<State.lanes.length;r++){
    ...
    if(lane.type===LANE.RAIL){
      if(lane.signal){
        ...
        const blink = lane.next<1000;
        lamp.material.emissiveIntensity = blink ? (Math.sin(performance.now()/120)>0? 0.6 : 0.15) : 0.15;
        lane.signal.position.z = rowToZ(r);
      }
      lane.next -= dt;
      if(lane.next<=0 && lane.ents.length===0){
        const t = new THREE.Mesh(assets.geos.geoTrain, assets.mats.matTrain);
        t.castShadow=true; t.userData.kind='train';
        t.position.set(lane.dir>0? leftX-6 : rightX+6, 0.5, rowToZ(r));
        world.add(t); lane.ents.push(t);
        lane.next = lane.cooldown + (Math.random()*1000);
      }
      const speed = lane.speed * TILE;
      lane.ents.forEach(m=> m.position.x += speed*dt/1000);
      lane.ents = lane.ents.filter(m=>{
        const alive = (lane.dir>0? m.position.x < rightX+8 : m.position.x > leftX-8);
        if(!alive) world.remove(m);
        return alive;
      });
    }
  }
...
function tryMove(dc,dr){
  ...
  const targetCol = Math.max(0, Math.min(COLS-1, p.col+dc));
  ...
  // اتجاه الوجه: أمام=0، خلف=π، يمين=+π/2، يسار=-π/2
  let aim;
...
container.addEventListener('touchend', e=>{
  ...
  if(absX<24 && absY<24){ tryMove(0,+1); }
...
function tick(ts){
  let dt = ts - lastRAF; lastRAF = ts; if (dt > 250) dt = 250;
  acc += dt; while (acc >= STEP) { update(STEP); acc -= STEP; }
  render();
...
shareBtn.addEventListener('click', async()=>{
  ...
  if(navigator.share){
    try{ await navigator.share({title:'Ketty Cross',text:'Play Ketty Cross — 3D',url}); }catch{}
  }else{
    try{
      await navigator.clipboard.writeText(url);
      shareBtn.textContent=(State.lang==='ar'?'تم نسخ الرابط':'Link copied');
      setTimeout(()=>shareBtn.textContent=LANG[State.lang].share,1200);
    }catch{ alert(url); }
  }
});
...
function reset(){
  clearEntities(State.lanes, world);
  coinMeshes.forEach(m=>{ if(m) world.remove(m); });
  coinMeshes.clear();
  laneMeshes.forEach(m=>{ if(m) world.remove(m); });
  laneMeshes.length=0;
