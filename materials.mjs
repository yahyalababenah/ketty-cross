...
function makeWoodTexStrong(renderer){
  const w=256,h=64; const cvs=document.createElement('canvas'); cvs.width=w; cvs.height=h;
...
function makeBarkTex(renderer){
  const w=64,h=128; const cvs=document.createElement('canvas'); cvs.width=w; cvs.height=h;
  const ctx=cvs.getContext('2d');
  for(let y=0;y<h;y++){
    for(let x=0;x<w;x++){
...
export function initMaterials(renderer){
  ...
  // مواد الأرضيات
  const matGrass = new THREE.MeshStandardMaterial({color:0xfafafa, roughness:0.95, metalness:0.0}); // رصيف/عشب فاتح
  const matRoad  = new THREE.MeshStandardMaterial({color:0x5f6f8f, roughness:0.88, metalness:0.05}); // إسفلت أزرق فاتح
  const matRail  = new THREE.MeshStandardMaterial({color:0x9aa0b4, roughness:0.7,  metalness:0.1});
  const matRiver = new THREE.MeshPhysicalMaterial({color:0x8fe3ff, roughness:0.18, metalness:0.05, transmission:0.0, clearcoat:0.5, clearcoatRoughness:0.4, transparent:true, opacity:0.98});
  const matFinish= new THREE.MeshStandardMaterial({color:0xffd166, roughness:0.6});

  // سيارات/قطارات/جذوع/عملة
  ...
  const geoLog  = new THREE.BoxGeometry(TILE*2.5, 0.42, 0.8);
  const geoTrain= new THREE.BoxGeometry(TILE*4.2, 0.7, 0.9);
